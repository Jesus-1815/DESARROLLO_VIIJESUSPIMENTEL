...
class TransactionManager {
    ...
    public function setIsolationLevel($level) {
        ...
        if (in_array($level, $levels)) {
            ...
            throw new Exception("Nivel de aislamiento no válido: $level");
        }
    }
    ...
    public function demonstrateDirtyRead() {
        try {
            ...
            $stmt = $this->pdo->prepare("UPDATE productos SET precio = precio * 1.1 WHERE id_producto = ?");
            $stmt->execute([1]);
    ...
    public function demonstrateRepeatableRead() {
        try {
            ...
            // Primera lectura
            $stmt = $this->pdo->prepare("SELECT precio FROM productos WHERE id_producto = ?");
            $stmt->execute([1]);
...
try {
    ...
    echo "Error en la gestión de transacciones: " . $e->getMessage();
}

$pdo = null; // Cerrar la conexión al final
?>
