...
function buscarLibros($biblioteca, $termino) {
    $termino = strtolower($termino); // Convertimos el término de búsqueda a minúsculas
...
function generarReporteBiblioteca($biblioteca) {
    ...
    $reporte['total_prestados'] = count(array_filter($biblioteca, function($libro) {
        return $libro['prestado'];
    }));
    ...
    $autorMasFrecuente = array_key_first($conteoAutores);
    $reporte['autor_con_mas_libros'] = $autorMasFrecuente;

    return $reporte;
}

// Ejemplo de uso de la función de reporte
$reporteBiblioteca = generarReporteBiblioteca($biblioteca);
echo "Reporte de la Biblioteca:\n";
print_r($reporteBiblioteca);
