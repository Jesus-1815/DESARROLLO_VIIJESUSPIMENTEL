...
function calcularCrecimiento($ventas) {
    ...
    $ultimaVenta = end($ventas); // Obtener la última venta
    if ($primeraVenta == 0) {
        return $ultimaVenta > 0 ? 100 : 0; // Evitar división por cero
    }
    return (($ultimaVenta - $primeraVenta) / $primeraVenta) * 100;
}

// 9. Calcular el crecimiento por región y producto
$mayorCrecimiento = 0;
$productoMayorCrecimiento = '';
$regionMayorCrecimiento = '';

echo "\nPorcentaje de crecimiento por región y producto:\n";
foreach ($ventas as $region => $productos) {
    echo "$region:\n";
    foreach ($productos as $producto => $ventasProducto) {
        ...
        // Verificar si es el mayor crecimiento
...
// 10. Mostrar el producto y región con mayor crecimiento
echo "\nProducto con mayor crecimiento: $productoMayorCrecimiento (Región: $regionMayorCrecimiento, Crecimiento: " . number_format($mayorCrecimiento, 2) . "%)\n";
